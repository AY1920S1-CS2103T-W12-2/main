...
//tag::setbudget[]
=== Setting budgets
We allow the user to maintain a `**Budget**` for each calendar month. This section details how `**THRIFT**` handle
requests made by the user who is trying to set a budget for their desired month. Each `**Budget**` stores a `**Calendar**`
and a `**BudgetValue**`. The following class diagram shows how a `**Budget**` object looks like:

image::add-budget/BudgetClassDiagram.png[]

==== Implementation
We store every `**Budget**` set by the user into `BudgetList#List<Budget>`, which is a list object in `**BudgetList**`.
This list will contain only one unique `**Budget**` object for each month, with the month uniqueness indicated by `Budget#period` in the "MM/YYYY" format.
Below is a class diagram explaining how the relations look:

image::add-budget/BudgetListClassDiagram.png[]

Whenever the user attempts to add a `**Budget**`, `**THRIFT**` will check if the `**Budget**` currently exists in `BudgetList#List<Budget>`.
`**BudgetList**` contains a few methods that simplifies such management of the list:

. `BudgetList#getBudgetForMonthYear(Calendar toCheck)` - Checks if `toCheck` matches any of the `Budget#period` in `List<Budget>`.
. `BudgetList#setBudget(Budget toSet)` - Adds `toSet` into `List<Budget>` if `toSet#period` does not match any of the `Budget#period` in the list. Otherwise,
overwrite the matching `**Budget**` in `List<Budget>` with `toSet`.
. `BudgetList#removeBudget(Budget toRemove)` - Removes `toRemove` from `List<Budget>`.
...
before, so `BudgetList#List<Budget>` is empty:
...
Since the user input is valid, the `**Budget**` is successfully created and inserted into `BudgetList#List<Budget>`.
...
`BudgetList#List<Budget>`, a check to see if a budget is already set for that month will occur. Since there is already a
budget set for October 2019, the existing budget is replaced by this newly created `**Budget**`.

The following code snippet is part of `BudgetList#setBudget(Budget toSet)` and elaborates on how this checking is done:
...
is huge. Need to look through the huge list to find the `**Budget**` to replace.

Again, we went with alternative 1 because we wanted to simulate the real world relations. Otherwise, there might be confusion
...
knowledge about how the encryption work as `**THRIFT**` will handle all encryption and decryption for them.
...
TIP: The encryption and decryption will take place every time after `LogicManager#execute()` is called.
// end::dataencryption[]
